...
<html>
  <head>
        ...
        function showVersions(){
			$.ajax( {  
		         url : "<%=basePath%>spareType/showSpareVersion.action",  
		         type : 'post',  
		         dataType : 'json',  
		...
		function showTreeFirst(){
			$.ajax( {  
		         url : '<%=basePath%>spareType/getAllSpareType.action?version='+$("#version").val(),  
		         type : 'POST',  
		         dataType : 'json',  
		         contentType:'application/json',  
		         async : false,  
        ...
        function saveForm(){
        	...
        	if($("#id").val()==""){		//新增的
				$.ajax({
					url:'<%=basePath %>spareType/saveSpareType.action',
					type:'post',
        		...
        		$("#form1").attr("action","<%=basePath %>spareType/saveSpareType.action");
        	}else{				//修改
        		$.ajax({
					url:'<%=basePath %>spareType/updateSpareType.action',
					type:'post',
					contentType: "application/x-www-form-urlencoded; charset=UTF-8", 
					data:{
						id:$("#id").val(),
        ...
        function deleteNode(){
        	if(!$("#id").val()==null ||$("#id").val()==""){
        		...
        		if(confirm("是否删除？")){
        		 $.ajax( {  
			         url : "<%=basePath%>spareType/deleteSpareType.action",  
			         type : 'GET',  
			         dataType:'text',
...
function addNewVersion(){
	if(confirm('是否确定复制当前备品备件类型版本并生成最新的版本？')){
		$.ajax({
			url:'<%=basePath%>spareType/copySpareCurrentVersion.action',
...
function publish(){
	if(confirm('是否确认发布改版本?')){
		$.ajax({
			url:'<%=basePath%>spareType/sparePublish.action',
			type:'post',
			data:{version:$("#version").val()},
			error:function(xmlHttpRequest,textStatus,errorThrown){
				alert(textStatus);
	...
	<div class="tHeader">
            <div class="th_inMarginWrap">
				...
				<ul class="th_medNav" id="reportArea">
					...
					<li class="cur"><span><a href="#" class="navLink" ><i><img src="<%=basePath %>css/default/images/icons_1.gif" alt=""><br>备品备件物资代码</i></a></span></li>
	...
	<div class="tc_inDataArea" id="main">
	    ...
	    <div class="clearfix">
	      ...
	      <div id="content" class="fl w70p" style="height:100%;width:70%;">
	        <form id="form1" method="post" action="<%=basePath %>spareType/updateSpareType.action">
	          <div class="clearfix b_bor">
	              <label for="textfield" class="fl p5 w120 lable_bg">分类名称</label>
			  ...
			  <div class="clearfix b_bor">
	              <label for="textfield" class="fl p5 w120 lable_bg">分类代码</label>
	              <div class="fl p5 w70p">
			   ...
			   <div class="clearfix b_bor">
	              ...
	              <div class="fl p5 w70p">
	                <input type="text" name="remark" id="remark" class="input_xxlarge"/>
	              </div>
			  </div>
	            <div class="t_c p5">
