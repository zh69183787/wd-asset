...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
        ...
        function showVersions(){
			$.ajax( {  
		         url : "<%=basePath%>assetDepartment/findAllVersion.action",  
		         type : 'post',  
		         dataType : 'json',  
		...
		function showTreeFirst(){
			$.ajax( {  
		         url : '<%=basePath%>assetDepartment/getAllDepartment.action?version='+$('#version').val(),  
		         type : 'POST',  
		         dataType : 'json',  
		         contentType:'application/json',  
		         async : false,  
        ...
        function saveForm(){
        	...
        	$.ajax({
				url:'<%=basePath %>assetDepartment/checkUniqueness.action',
				type:'post',
				data:{
					id:$('#id').val(),
					code:$('#code').val(),
				...
				success:function(data){
					if(data==false){
						alert('维护部门名称或代码重复！请检查后重新输入！');
					}else{
						$.ajax({
							url:'<%=basePath %>assetDepartment/saveDepartment.action',
							type:'post',
							...
							success:function(){
								alert('保存成功！');
								window.location.href='<%=basePath%>jsp/basecode/department.jsp?version='+$('#version').val();
							}
						});
					}
				}
        ...
        function deleteNode(){
        	if(!$('#id').val()==null ||$('#id').val()==""){
        		...
        		if(confirm("是否删除？")){
        		 $.ajax( {  
			         url : "<%=basePath%>assetDepartment/deleteDepartment.action",  
			         type : 'post',  
			         data:{id:$('#id').val()}, 
			         async : false,  
			         success : function() {
...
function addNewVersion(){
	if(confirm('是否确定复制当前资产类型版本并生成最新的版本？')){
		$.ajax({
			url:'<%=basePath%>assetDepartment/copyCurrentVersion.action',
...
function publish(){
	if(confirm('是否确认发布改版本?')){
		$.ajax({
			url:'<%=basePath%>assetDepartment/publish.action',
			type:'post',
			data:{version:$('#version').val()},
			error:function(xmlHttpRequest,textStatus,errorThrown){
				alert(textStatus);
	...
	<div class="tHeader">
            <div class="th_inMarginWrap">
				...
				<ul class="th_medNav" id="reportArea">
					...
					<li class="cur"><span><a href="#" class="navLink"><i><img src="<%=basePath %>css/default/images/icons_1.gif" alt=""><br>维护部门管理</i></a></span></li>
      ...
      <div id="content" class="fl w70p" >
        <form id="form1" method="post" action="<%=basePath %>assetType/updateAssetType.action">
          ...
          <div class="clearfix b_bor">
              <label for="textfield" class="fl p5 w120 lable_bg">维护部门名称</label>
